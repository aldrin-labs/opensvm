...
export function calculateSeverity(
  ...
  adjustedScore *= confidenceScore * mlWeight;  // Statistical deviation adjustments
  if (pattern.type === 'suspicious_fee_spike' && context.feeStatistics) {
    ...
    if (AnomalyStatistics.isStatisticalOutlier(feeValue, context.feeStatistics.mean, context.feeStatistics.stdDev, 2)) {
      ...
      console.log(`🐛 Fee spike outlier detected, added 0.5: ${adjustedScore}`);
    }
  }
  ...
  if (context.transactionStatistics && eventFrequency > 1) {
    ...
    if (trendAnalysis.isAnomaly) {
      ...
      console.log(`🐛 Trend anomaly detected, added ${trendAnalysis.trendStrength * 0.5}: ${adjustedScore}`);
    }
  }
  ...
  if (context.baselineData && context.baselineData.dataPoints > 50) {
    ...
    if (currentErrorRate > baselineErrorRate * 2) {
      ...
      console.log(`🐛 Baseline deviation detected (${currentErrorRate} vs ${baselineErrorRate}), added 0.3: ${adjustedScore}`);
    }
  }

  if (eventFrequency > 10) {
    adjustedScore += 1;
    console.log(`🐛 High event frequency (${eventFrequency}), added 1: ${adjustedScore}`);
  }
  if (pattern.category === 'security') {
    adjustedScore += 0.5;
    console.log(`🐛 Security category, added 0.5: ${adjustedScore}`);
  }
  if (pattern.category === 'financial') {
    adjustedScore += 0.3;
    console.log(`🐛 Financial category, added 0.3: ${adjustedScore}`);
  }

  console.log(`🐛 Final adjustedScore: ${adjustedScore}`);
  ...
  if (adjustedScore >= 1.8) {
    console.log(`🐛 Returning: medium`);
    return 'medium';
  }
