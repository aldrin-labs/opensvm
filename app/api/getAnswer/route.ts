import Together from "together-ai";
    ...
    You are an expert Solana blockchain analyst with access to comprehensive
    RPC
    ...
    When responding to queries, always consider both API sources and suggest
    the
    ...
    - Use appropriate commitment levels (processed, confirmed, finalized)
    based
    on use case
    ...
    class StabilityMonitor {
      ...
      static getMetrics() {
        this.checkReset();
        const successRate = this.requests > 0 ? ((this.requests -
    this.failures)
    ...
    tools-only mode.");
      ...
      // Overall request timeout of 120 seconds (3x increase, leave 15
    seconds
    buffer for cleanup)
    ...
    'HEALTHY' : 'DEGRADED'}`);
          ...
          // If ownPlan mode is enabled, skip tool execution and go directly
    to
    planning
          if (ownPlan) {
    ...
    requestStart);
          ...
          if (toolResult.handled && toolResult.response) {
            ...
            console.log(`✅ Tool handling successful in
    ${processingTime}ms`);
    ...
    *Error: ${(error as Error).message}*`;
    ...
    prompt (${systemPromptToUse.length} chars)`);
      ...
      try {
        ...
        console.log(`⏱️  Starting LLM request at ${new
    Date().toISOString()}`);
    ...
    ${errorText}`);
        ...
        let plan = answer.choices?.[0]?.message?.content || "Failed to
    generate
    plan";
    ...
    used
        ...
        return new Response(plan, {
          status: 200,
          headers: {
            "Content-Type": isCustomPrompt ? "text/plain" :
    "application/xml",
            "Cache-Control": "no-cache",
            "X-Processing-Time": `${processingTime}ms`,
    ...
    // ✅ NEW: Default OVSM system prompt (XML-based for backward
    compatibility)
    ...
    partialData?: any): Promise<Response> {
      ...
      // Fallback: use LLM (Together) to craft an answer if no tool handled
    it
    ...
    **IMPORTANT: NO EMOJIS, only neat ascii**
    ...
    APIs
      ...
      Focus on being helpful while matching the user's communication style
    and
    energy level.
    ...
    endpoint) in the answer.
      ...
      Incorporate these steps into the final response, presenting the
    computed
    ...
    executed before failing. Use this available data in your response:\n\n`;
        ...
        systemPrompt += `Use this partial data to provide the most
    comprehensive
    ...
    second LLM timeout
        ...
        let parsedAnswer: any = answer.choices?.[0]?.message?.content ||
    "Failed
     to get answer";
    ...
    formatting
        ...
        parsedAnswer =
    generativeCapability.postProcessResponse(parsedAnswer);
        ...
        const processingTime = Date.now() - requestStart;
        console.error(`❌ LLM processing failed after ${processingTime}ms:`,
    e);
    /whats?\s*up/,
