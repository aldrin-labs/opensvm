// Binance-compatible Price Ticker endpoint
// GET /api/v3/ticker/price - Latest price for a symbol or symbols

import { NextRequest, NextResponse } from 'next/server';
import { TickerPrice, BinanceError } from '@/lib/trading/binance-types';

export const runtime = 'edge';

// Cache for price data
const priceCache = new Map<string, { data: TickerPrice; timestamp: number }>();
...
async function getTickerPrice(symbol: string): Promise<TickerPrice | null> {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (symbol) {
      ...
      priceCache.set(symbol, { data: ticker, timestamp: Date.now() });
    ...
    const tickerPromises = symbolList.map(async (sym) => {
      ...
      const ticker = await getTickerPrice(sym);
      if (ticker) {
        priceCache.set(sym, { data: ticker, timestamp: Date.now() });
      }
      return ticker;
    });
