...
async function waitForTransactionGraph(page: any) {
  try {
    ...
    const graphTab = page.locator('button[data-value="graph"]');
    const tabCount = await graphTab.count();
    
    if (tabCount === 0) {
      console.log('Graph tab not found, checking all available tabs...');
      const allTabs = page.locator('button[data-value]');
      const tabValues = await allTabs.evaluateAll((buttons: any) => 
        buttons.map((btn: any) => btn.getAttribute('data-value'))
      );
      console.log('Available tabs:', tabValues);
...
test.describe('TransactionGraph Component', () => {
  test.beforeEach(async ({ page }) => {
    // Navigate to the real transaction page
    console.log('Navigating to transaction page:', `/tx/${TEST_TRANSACTION}`);
    ...
    // Check for tabs
    const tabElements = page.locator('button[data-value]');
    const tabCount = await tabElements.count();
    console.log(`Tab elements found: ${tabCount}`);
    if (tabCount > 0) {
      const tabValues = await tabElements.evaluateAll((buttons: any) => 
        buttons.map((btn: any) => btn.getAttribute('data-value'))
      );
      console.log('Available tabs:', tabValues);
    }
