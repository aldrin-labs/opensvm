...
import type { PerfSample } from '@solana/web3.js';
import { NETWORK_PERFORMANCE_KNOWLEDGE } from '../core/knowledge';

export class NetworkCapability extends BaseCapability {
    type = 'network' as const;

    private readonly NETWORK_TERMS = ['network', 'tps', 'transactions per second', 'validator', 'performance', 'block time', 'load'];
    ...
    tools = [
        ...
        },
        {
                ...
                async () => {
                    return this.executeWithConnection(async (connection) => {
                        ...
                        if (!Array.isArray(samples) || samples.length === 0) {
                            return { message: 'Unable to retrieve network performance data.' };
                        }
                        ...
                        if (validSamples.length === 0) {
                            return { message: 'No valid performance samples available.' };
                        }
            ...
            matches: (message: Message) => {
                ...
                return content.includes('tps') || content.includes('transactions per second') || content.includes('performance') || content.includes('network load');
            },
            required: true
        }
    ];
