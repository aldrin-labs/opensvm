...
export const DataTypeFilters: React.FC<DataTypeFiltersProps> = ({
  ...
  return (
    <div className="space-y-3">
      <h4 className="font-medium text-sm text-foreground/80">Filter by Data Type</h4>
      <div className="space-y-2">
        {['transactions', 'blocks', 'programs', 'tokens'].map((type) => (
          <div key={type} className="flex items-center">
            ...
            />
            <label htmlFor={`type-${type}`} className="ml-2 text-sm text-foreground/80 capitalize">
              {type}
            </label>
          </div>
        ))}
