...
export async function addAccountToGraph(
  ...
  // Check if this account has SPL transfers to determine effective max depth
  ...
  if (depth === 0) {
    try {
      ...
      if (!hasSplTransfers) {
        effectiveMaxDepth = Math.min(1, maxDepth); // Limit depth for non-SPL accounts
        console.log(`⚠️ [GRAPH_BUILD] Account ${address} has no SPL transfers, limiting depth to ${effectiveMaxDepth}`);
      } else {
        console.log(`✅ [GRAPH_BUILD] Account ${address} has SPL transfers, using full depth ${maxDepth}`);
      }
    } catch (error) {
      console.warn(`❌ [GRAPH_BUILD] SPL check failed for ${address}, using default depth:`, error);
  ...
  if (depth >= effectiveMaxDepth) {
    console.log(`🛑 [GRAPH_BUILD] Reached max depth ${effectiveMaxDepth} for ${address}, stopping`);
    return;
  }
